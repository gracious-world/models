...
class ActivityTask extends BaseModel {
    ...
    public function isValidateTask($sTime = null) {
        ...
        return $this->activity()->first()->isAvailable($now);
    }
...
//                die("ccc\n");
        ...
        if (!$userTask->id){
//            pr($this->toArray());
            $oCondition = $this->conditions()->first();
            pr($oCondition->toArray());
            $aTaskData = json_decode($oCondition->params,true);
            $aTaskData['deposit_amount'] = $data['amount'];
            $aTaskData['start_time'] = Carbon::now()->toDateTimeString();
            exit;
//            $userTask->data = [
//                'amount' => 
//            ];
...
//        exit;
        ...
        pr($userTask->isFinished($userTask));
                die("ccc\n");
        exit;
        if ($userTask->isFinished($userTask)) {
            return true;
        }
     ...
     *
     */
    public function completeOld($user_id) {
        $this->validationErrors = new MessageBag();
        //过滤掉无效活动
        if (!$this->isValidateTask()) {
            $this->validationErrors->add('start_time', "Task [{$this->id}] has been expired");
        ...
        //如果活动需要报名而未报名则返回失败
        $userTask = ActivityUserTask::firstOrNew($aCondition);
//        $userTask->id or $userTask->save();
//        pr($userTask->getTable());
//        pr($userTask->toArray());
//        exit;
        //过滤掉需要报名,而没有报名的情况
        if ($this->need_sign && !$userTask->is_signed) {
        ...
        //过滤掉已完成的任务
        if ($this->isFinished($userTask)) {
            return true;
        }
//        die('continue');
        //循环判断任务内的条件情况
        foreach ($this->conditions()->get() as $condition) {
            pr($condition->toArray());
//            continue;
            $aCondition = [
                'user_id' => $user_id,
            ...
            pr($userCondition->toArray());
            if (!$this->isFinished($userCondition)) {
                $obj = FactoryClass::Factory($condition)->load($userCondition);
                pr($obj);
                exit;
...
//        完成则发放礼物
     ...
     * @return bool
     */
    public function send($user_id, $aExtraData = null, $sSource = ActivityUserBonus::SOURCE_TASK_SYSTEM) {
        foreach ($this->taskPrizes()->get() as $taskPrize) {
            //是否自动生成奖品
            if ($sSource == ActivityUserBonus::SOURCE_TASK_SYSTEM && $taskPrize->is_auto_send == 0) {
                continue;
            }
     ...
     *
     */
    public function isFinished(ActivityTaskTypeInterface $userObject) {
        /**
