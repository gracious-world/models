...
//        'left_count' => 'integer',
    ...
    protected function getFriendlyTypeAttribute() {
        return __('_activityrule.' . static::$aTypes[$this->type]) . '(' . __('_activityrule.' . static::$aGenerateTypes[$this->generate_type]) . ')';
    }

    protected function getFriendlyGenerateTypeAttribute() {
        return __('_activityrule.' . static::$aGenerateTypes[$this->generate_type]);
    }
    ...
    public static function findAvailableRules(Activity $oActivity, $iLocker) {
        ...
        $aActivityRule = static::where('activity_id', '=', $oActivity->id)
...
//        dd(DB::getQueryLog());
    ...
    public function deduct($iLocker, $iQuantity = 1) {
        $iAffectRows = static::where('id', '=', $this->id)
                ->where('locker', '=', "$iLocker") // 已被锁定
...
//        $iAffectRows = static::where('activity_id', '=', $oActivity->id)
