...
class RoleUser extends BaseModel {
     ...
     * @return array
     */
    public static function getRolesOfUser($iUserId) {
        $bReadDb   = true;
        $bPutCache = false;
        if (static::$cacheLevel != self::CACHE_LEVEL_NONE) {
            Cache::setDefaultDriver(static::$cacheDrivers[static::$cacheLevel]);
            $sCacheKey = static::compileRolesOfUserCacheKey($iUserId);
            if ($aRoles    = Cache::get($sCacheKey)) {
                $bReadDb = false;
            }
            else {
                $bPutCache = true;
        ...
        if ($bReadDb) {
            $aRoles = static::where('user_id', $iUserId)->lists('role_id');
        }
        if ($bPutCache) {
            Cache::forever($sCacheKey, $aRoles);
        }
//        pr($aRoles);
//        exit;
        return $aRoles;
     ...
     * @return string
     */
    private function compileRolesOfUserCacheKey($iUserId) {
        return static::getCachePrefix(true) . 'of-user-' . $iUserId;
    }
}
