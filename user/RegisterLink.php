...
class RegisterLink extends BaseModel {
    ...
    protected function getAgentQqsFormattedAttribute() {
        $s = $this->attributes['agent_qqs'] or $s = '-';
        return $s;
    }
...
//         pr((($aPrizeGroupSets)));
//         pr($aPrizeGroupWaters);
//         exit;
        $aPrizeGroups    = [];
        $aPres           = ['series_id', 'series_id'];
...
//            $sWater = $aPrizeGroupWaters[$value->prize_group];
            $aPrizeGroups['series-' . $value->series_id] = ['prize_group' => $value->prize_group];
        }
