...
//        return static::where('id', '=', $this->id)->where('status', '=', self::STATUS_IN_USE)->update($aExtraData) > 0;
    ...
    public static function getLastUpdatedTime($iUserId) {
        return static::where('user_id', '=', $iUserId)->max('updated_at');
    }

    public static function _setStatus($iUserId, $iFromStatus, $aExtraData = []) {
        #update by zero 增加缓存处理
        $oQuery         = static::where('user_id', '=', $iUserId)->where('status', '=', $iFromStatus);
        $oUserBankCards = $oQuery->get();
        if (!$oUserBankCards->count()) {
            return false;
        }
        if ($bSucc = ($oQuery->update($aExtraData) > 0)) {
            //银行卡数量缓存处理
            $sKey = static::createUserBankCardCountCacheKey($iUserId);
            Cache::setDefaultDriver(static::$cacheDrivers[static::$cacheLevel]);
            !Cache::has($sKey) or Cache::forget($sKey);
            //基础缓存处理
            foreach ($oUserBankCards as $oUserBankCard) {
                $sBasicKey = static::createCacheKey($oUserBankCard->id);
                Cache::setDefaultDriver(static::$cacheDrivers[static::$cacheLevel]);
                !Cache::has($sBasicKey) or Cache::forget($sBasicKey);
            }
        }
        return $bSucc;
    }
