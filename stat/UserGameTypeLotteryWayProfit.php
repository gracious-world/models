 ...
 * @author Garin
 */
class UserGameTypeLotteryWayProfit extends BaseModel {

    protected $table = 'user_gt_lottery_way_profits';
    public static $resourceName = 'UserGameTypeLotteryWayProfit';
    ...
    public static $mainParamColumn = 'game_type';
     ...
     * @param $iWayId
     *
     * @return bool|UserGameTypeLotteryWayProfit|static
     */
    public static function getUserProfitObject($sDate, $iUserId, $iGameType, $iLotteryId, $iWayId) {
        ...
        if (!is_object($obj)) {
            ...
            $obj = new static($data);
            if (!$obj->save()) {
                    var_dump($obj->getValidationErrorString());
                    return false;
            }
            $obj = self::getUserProfitObject($sDate, $iUserId, $iGameType, $iLotteryId, $iWayId);
        } else {
     ...
     * @param        $iGameType   游戏类别
     *
     * @return array
     */
    public static function getUserProfitByDate($sBeginDate, $sEndDate, $iUserId, $iGameType) {
     ...
     * @return float        销售总额
     */
    public static function getUserTotalTurnover($sBeginDate, $sEndDate, $iUserId, $iGameType) {
        $aUserProfits = static::getUserProfitByDate($sBeginDate, $sEndDate, $iUserId, $iGameType);
        $aTurnovers = [];
     ...
     * @return float        用户盈亏
     */
    public static function getUserTotalProfit($sBeginDate, $sEndDate, $iUserId, $iGameType) {
        $aUserProfits = static::getUserProfitByDate($sBeginDate, $sEndDate, $iUserId, $iGameType);
        $aProfits = [];
        foreach ($aUserProfits as $data) {
    ...
    public static function getUserGoodAtLotteryWay($iUserIds, $iGameType = 0, $sBeginDate = '', $sEndDate = '') {
        $aConditions = [];
        ...
        if (!empty($iGameType)){
            $aConditions['game_type'] = ['=', $iGameType];
        }
        ...
        foreach ($oUserLotteryWays as $key => $oLotteryWay) {
            if (!isset($aUserGoodAtLotteryWay[$oLotteryWay->user_id]) ||
                ...
                $aUserGoodAtLotteryWay[$oLotteryWay->user_id]['title'] =  '';
                if(empty($oLotteryWay->way)){
                    !isset($oLotteryWay->way_id) or $oSeriesWay = SeriesWay::find($oLotteryWay->way_id);
                    !isset($oSeriesWay->series_id) or $oSeries = Series::find($oSeriesWay->series_id);
                    !isset($oSeries->name) or $aUserGoodAtLotteryWay[$oLotteryWay->user_id]['title'] = $oSeries->name ;
                    !isset($oSeriesWay->name) or $aUserGoodAtLotteryWay[$oLotteryWay->user_id]['title'] .= '-' . $oSeriesWay->name;
    ...
    public static function updateTurnOver($sDate, $iGameType, $oUser, $fAmount) {
        return static::updateProfitData('turnover', $sDate, $iGameType, $oUser, $fAmount);
    }

    public static function updatePrize($sDate, $iGameType, $oUser, $fAmount) {
        return static::updateProfitData('prize', $sDate, $iGameType, $oUser, $fAmount);
    }

    public static function updateBonus($sDate, $iGameType, $oUser, $fAmount) {
        return static::updateProfitData('bonus', $sDate, $iGameType, $oUser, $fAmount);
    }

    public static function updateCommission($sDate, $iGameType, $oUser, $fAmount) {
        return static::updateProfitData('commission', $sDate, $iGameType, $oUser, $fAmount);
    }

    public static function updateProfitData($sType, $sDate, $iGameType, $iLotteryId, $iWayId, $oUser, $fAmount) {
        $sFunction = 'add' . ucfirst(String::camel($sType));
        $oProfit = self::getUserProfitObject($sDate, $oUser->id, $iGameType, $iLotteryId, $iWayId);
