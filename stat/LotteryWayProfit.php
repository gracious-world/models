 ...
 * @author system
 */
class LotteryWayProfit extends BaseModel {

    protected $table                         = 'lottery_way_profits';
    ...
    public static $resourceName              = 'LotteryWayProfit';
     ...
     * @return UserProfit
     */
    public static function getProfitObject($sDate, $iLotteryId, $iWayId) {
...
//        pr($this->toArray());
    ...
    public static function updateProfitData($sType, $sDate, $iLotteryId, $iWayId, $oUser, $fAmount) {
        $oLotteryWayProfit = self::getProfitObject($sDate, $iLotteryId, $iWayId);
        if (!is_object($oLotteryWayProfit)) {
            return false;
        }
        $sFunction = 'add' . ucfirst($sType);
        $bSucc     = true;
        return $oLotteryWayProfit->$sFunction($fAmount, $oUser->is_tester);
    }
