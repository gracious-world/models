...
class PrjPrizeSet extends BaseModel
{
    ...
    {
        $oPrjPrizeSet = new static($aSetting);
    ...
    {
        return static::where('project_id', '=', $iProjectId)->where('status', '=', $iFromStatus)->where('status', '<>', $iToStatus)->update(['status' => $iToStatus]);
    }
    ...
    {
        return static::where('project_id', '=', $iProjectId)->where('status', '=', $iStatus)->count();
    }
    ...
    public static function getCountOfIssue($iLotteryId, $sIssue, $iStatus = self::STATUS_WAIT)
    {
        ...
        return static::doWhere($aConditions)->count();
    }
    ...
    {
        static::compileData($oProject, $aPrize);
...
//            pr($aPrizeSetFromRedis);
        ...
        if (!$bHasInRedis || $iNeedCount > 0) {
            ...
            $aMorePrizeSet = static::where('status', '<>', self::STATUS_DROPED)->where('prize', '>=', 1000)->orderBy('id', 'desc')->limit($iCount)->get(['username', 'prize', 'lottery_id', 'created_at']);
            foreach ($aMorePrizeSet as $oMorePrize) {
                $aPrizeSet[] = $oMorePrize;
                $redis->rpush($sKey, json_encode($oMorePrize->toArray()));
            }
        }
//        pr($aPrizeSet);
//        exit;
    ...
    {
        return static::getCachePrefix(true) . 'list';
    }
...
//        ];
////        is_null($iStatus) or $aConditions['status'] = ['=', $iStatus];
//        return static::where('status','<>',self::STATUS_DROPED)->orderBy('id','desc')->limit($iCount)->get(['username','prize','lottery_id']);
//    }
...
//        pr($rules);
    ...
    public static function getTotalAmount($iUserId, $dBeginTime, $dEndTime) {
