...
//        $iBetThreadsLength = count($aBetThreads);
    ...
    public static function getThreadList($iLotteryId,$sIssue){
        $aThreadIds = [];
        if (static::validateBetThread($iLotteryId,$sIssue,-1)){
            $aAllThreads = static::where('lottery_id','=',$iLotteryId)->where('issue','=',$sIssue)->get(['thread_id']);
    ...
    public static function addThread($iLotteryId,$sIssue,& $iThreadId){
        ...
        $iResult   = 0;
        if (static::validateBetThread($iLotteryId,$sIssue,$iThreadId)){
            $aThread = [
                'lottery_id' => $iLotteryId,
                'issue'      => $sIssue,
                'thread_id'  => $iThreadId
            ];
            $iResult = static::insert($aThread);
        }
        return (bool) $iResult;
    }
...
//        is_array($aThreadIds) or $aThreadIds = [$aThreadIds];
        return static::whereIn('thread_id',$aThreadIds)->delete() > 0;
    }
