...
//                return self::ERRNO_TRACE_ERROR_SAVE_ERROR;
    ...
    public function terminate($iType = self::TERMINATE_BY_USER){
        ...
        if ($iType == self::TERMINATE_BY_USER){
            $oDetails = $this->getDetails(null, TraceDetail::STATUS_WAITING);
        ...
        $aDetailCancelStatuses = [
            self::TERMINATE_BY_USER => TraceDetail::STATUS_USER_CANCELED,
            self::TERMINATE_BY_ADMIN => TraceDetail::STATUS_ADMIN_CANCELED,
            self::TERMINATE_BY_SYSTEM => TraceDetail::STATUS_SYSTEM_CANCELED,
        ];
        $iDetailToStatus = $aDetailCancelStatuses[$iType];
        if (!TraceDetail::setAllCanceled($this->id,$iDetailToStatus)){
...
//        return static::doWhere($aConditions)->update($data) > 0;
    }
...
//    }
    ...
    public function cancelDetail($aDetailIds){
        ...
        foreach ($aDetailIds as $iDetailId){
            if (!$oDetail = TraceDetail::find($iDetailId)){
                return false;
            }
