...
class TraceDetail extends BaseModel {
    ...
    public static $resourceName              = 'TraceDetail';
    public static $amountAccuracy            = 4;
    public static $htmlOriginalNumberColumns = [ 'multiple'];
...
//        pr($aDetails);
        foreach ($aDetails as $sIssue => $iMultiple) {
            $aDetail      = static::makeDetailData($oTrace, $sIssue, $iMultiple, $aEndTimes[$sIssue]);
    ...
    function generateProject($oTrace, $oUser, $oAccount, & $iErrno, $oBetTime = null) {
        ...
        $aAttributes['terminal_id']         = $oTrace->terminal_id;
        $aAttributes['way_id']              = $oTrace->way_id;
        $aAttributes['position']            = $oTrace->position;
...
//            pr($iReturn);
        ...
        $aExtradata  = [
            'terminal_id' => $oTrace->terminal_id,
...
//            return Trace::ERRNO_PRJ_GERENATE_FAILED_PRJ_ERROR;
    ...
    public static function setCanceled($bBySystem = false) {
        ...
        return static::setStatus($iToStatus, self::STATUS_WAITING);
    }
    ...
    protected static function setStatus($iToStatus, $iFromStatus) {
        return static::where('id', '=', $this->id)->where('status', '=', $iFromStatus)->where('status', '<>', $iToStatus)->update(['status' => $iToStatus]);
    }
...
//            ->where('end_time','>=',$time)
    ...
    protected static function setAllStatus($iTraceId, $iToStatus, $iFromStatus) {
        return static::where('trace_id', '=', $iTraceId)->where('status', '=', $iFromStatus)->where('status', '<>', $iToStatus)->update(['status' => $iToStatus]);
    }
    ...
    public static function getListByTraceId($iTraceId, $iPageSize = 15) {
        ...
        return static::where('trace_id', '=', $iTraceId)->paginate($iPageSize);
    }
    ...
    public function cancel($iType = 0) {
        ...
        if ($data['canceled_at'] && (strtotime($data['canceled_at']) > $this->end_time)){
            return false;
        }
        return static::where('id', '=', $this->id)->where('status', '=', self::STATUS_WAITING)->update($data) > 0;
    }

    public static function getUnGenerateDetailCount($iLotteryId, $sIssue) {
        return static::where('lottery_id', '=', $iLotteryId)->where('issue', '=', $sIssue)->where('status', '=', self::STATUS_WAITING)->get(['id'])->count();
    }

    public static function getUnGenerateTraces($iLotteryId, $sIssue) {
        return static::where('lottery_id', '=', $iLotteryId)->where('issue', '=', $sIssue)->where('status', '=', self::STATUS_WAITING)->lists('trace_id');
    }
