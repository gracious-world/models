...
//        exit;
     ...
     * @return array
     */
    public function checkPrizeSSC($oSeriesWay,$sBetNumber,$sPosition = null){
//        pr($sBetNumber);
//        exit;
        $aPrized = [];
        $aSpecialWay = [
...
//        pr($this->function);
//        exit;
        if (in_array($this->function , $aSpecialWay)) {
...
//            pr($aPrizeLevels);
//            exit;
            if ($iCount = $oBasicMethod->checkPrize($oSeriesWay, $this, $sWnNumber, $sBetNumberOfMethod, $sPosition)) {
                pr($iCount);
...
//                            pr($aPrizeLevels);
//                            exit;
                            $iBasicMethodId = $oSeriesMethod->basic_method_id;
                            foreach($iCount as $sRule){
                                $iLevel = $aPrizeLevels[$sRule];
                                isset($aPrized[ $iBasicMethodId ][$iLevel]) or $aPrized[ $iBasicMethodId ][$iLevel] = 0;
                                $aPrized[ $iBasicMethodId ][$iLevel] ++;
                            }
                    }
//                    pr($oBasicMethod->toArray());
//                    pr($oBasicMethod->wn_function);
//                    exit;
                }
                else{
                    $iLevel = count($aPrizeLevels) == 1 ? 1 : $aPrizeLevels[intval($sWnNumber)];
