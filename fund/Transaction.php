...
class Transaction extends BaseModel
{
    ...
    public static $iUsrPanelCount = 30;
...
//        exit;
    ...
    public static function getTotalAmount($iType, $iUserId, $dBeginTime, $dEndTime)
    {
        $dBeginTime or $dBeginTime = '1970-01-01';
        return static::whereBetween('created_at', [$dBeginTime, $dEndTime])
     ...
     *
     * @param       $sDate
     * @param array $aLotteryIds 没有游戏类别用游戏id限制类别
     * @param array $aWhere      其他条件
     *
     * @return mixed
     */
    public static function  & getLotteryTransactionData($sDate, $aLotteryIds = [], $aWhere = [])
    {
        $sEndDate = date('Y-m-d H:i:s', strtotime("{$sDate} +1 day -1 second"));
        if (!empty($aWhere)) {
            $oResults = static::doWhere($aWhere);
            $oResults = $oResults->whereBetween('created_at', [$sDate, $sEndDate]);
        } else {
            $oResults = static::whereBetween('created_at', [$sDate, $sEndDate]);
        }

        if (!empty($aLotteryIds)) {
            $oResults = $oResults->whereIn('lottery_id', $aLotteryIds);
        }

        $oResults = $oResults->select(DB::raw('user_id, username, is_tester, user_forefather_ids, type_id, sum(amount) amount'))
            ->groupBy('user_id')
            ->groupBy('type_id')
            ->get();
        return $oResults;
     ...
     * @param string $sDate
     * @param array $aTypes
     * @param bool $bOnlyTester
     * @param array $aLotteryIds 没有游戏类别用游戏id限制类别
     * @param array $aWhere      其他条件
     *
     * @return mixed
     */
    public static function & getTotalAmountOfTypes($sDate, $aTypes = [], $bOnlyTester = false, $aLotteryIds = [], $aWhere = [])
    {
        $sEndDate = date('Y-m-d H:i:s', strtotime("$sDate +1 day -1 second"));

        if (!empty($aWhere)) {
            $oResults = static::doWhere($aWhere);
            $oResults = $oResults->whereIn('type_id', $aTypes)
                ->whereBetween('created_at', [$sDate, $sEndDate]);
        } else {
            $oResults = static::whereIn('type_id', $aTypes)
                ->whereBetween('created_at', [$sDate, $sEndDate]);
        }

        if ($bOnlyTester) {
            $oResults = $oResults->where('is_tester', '=', 1);
        }
        ...
        $oResults = $oResults->select(DB::raw('lottery_id, type_id, sum(amount) amount '))
            ->groupBy('lottery_id')
            ->groupBy('type_id')
            ->orderBy('lottery_id')
            ->orderBy('type_id')
     ...
     * @return mixed
     */
    public static function & getTotalAmountOfDayTypes($sDate, $aTypes = [], $bOnlyTester = false)
    {
        $oResults = Transaction::whereIn('type_id', $aTypes);
        if ($bOnlyTester) {
            $oResults = $oResults->where('is_tester', '=', 1);
        }
        $oResults = $oResults->where('created_at', 'LIKE', $sDate . '%');
        $oResults = $oResults->select(DB::raw('type_id, sum(amount) amount '))
            ->groupBy('type_id')
            ->orderBy('type_id')
            ->get();
     ...
     * @return  object
     */
    public static function getForJcAddPrzie($iUserId, $sNote)
    {
        return Transaction::where('user_id', $iUserId)->where('note', $sNote)->get();;
    }
}
