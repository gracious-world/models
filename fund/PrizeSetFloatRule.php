...
//        'prize_group_set' => 'required|max:10240',
    ...
    public static function & getRuleList() {
        ...
        $oOriginRules = static::all($aFields);
        $aData = [];
    ...
    public static function getRuleObject($bUp, $iDays, $iClassicPrize) {
        ...
        $oRule = static::doWhere($aConditons)->first();
        if (!is_object($oRule)) {
            $data = [
                'is_up' => intval($bUp),
                'days' => $iDays,
    ...
    public static function updateRule($bUp, $iDays, $iClassicPrize, $iTurnOver) {
        $oRule = static::getRuleObject($bUp, $iDays, $iClassicPrize);
        $oRule->turnover = $iTurnOver;
        return $oRule->save(static::$rules);
    }

    public static function deleteDataByDayUp($bUp, $iDay) {
        return static::where('is_up', '=', $bUp)->where('days', '=', $iDay)->delete();
    }
    ...
    public static function getRulesByDayRange($sLastCalculateFloatDate) {
        $oOriginRules = static::getRuleList();
        $aData = [];
        if ($sLastCalculateFloatDate) {
            $bUp = $bDown = true;
            $iDay = Date::between(date('Y-m-d H:i:s', time()), $sLastCalculateFloatDate);
