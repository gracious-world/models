...
//        }
    ...
    public static function lockByUserId($iUserId, & $iLocker) {
        ...
        if ($iCount > 0) {
            $iLocker = $iThreadId;
            return static::where('user_id', '=', $iUserId)->first();
        }
        return false;
    }
...
//        exit;
    ...
    private static function getLocker($id) {
        ...
        return static::where('id', '=', $id)->pluck('locked');
    }
...
//        exit;
    ...
    public function clearProhibit() {
        if ($this->prohibit_amount > 0) {
            return $this->update(['prohibit_amount' => 0]);
        }
        return true;
    }
}
