...
class CalculatePrize extends BaseTask {
    ...
    protected $needGenerateDetailTraces = [];
    protected $needStopTraces = [];
        ...
        if (!$iProjectCount = ManProject::getUnCalcutatedCount($oIssue->lottery_id, $oIssue->issue)) {
            if ($this->setCalculateFinished($oIssue, 0, $bFinished)) {
                ...
                if ($this->hasWatingTraceDetail) {
                    $this->setTraceTask($this->needStopTraces, $this->needGenerateDetailTraces);
                }
                if ($bFinished) {
                    $oIssue->status_prize == ManIssue::PRIZE_FINISHED or $this->setFinishedSendMoneyTask();
                    $oIssue->status_trace_prj == ManIssue::TRACE_PRJ_FINISHED or $this->setFinishedTracePrjTask();
...
//            pr($oProject->toArray());
        if ($aPrized) {
            ...
            $DB->beginTransaction();
//            $aPrizeDetailOfPrj = [];
            if ($bSucc = $oProject->setWon($oIssue->wn_number, $aPrized, $aPrizeDetailOfPrj, $oTrace, $oRiskProject)) {
                $DB->commit();
                !is_object($oRiskProject) or $oRiskProject->setNewFlag();
//                $aPrizeDetails  = array_merge($aPrizeDetails,$aPrizeDetailOfPrj);
     ...
     */
    public static function setSendPrizeTask($aWonProjects) {
        if (empty($aWonProjects)) {
            return true;
        }
        return static::addTask('SendPrize', ['projects' => $aWonProjects], 'prize');
    }
     ...
     * @return bool
     */
    public static function setSendCommissionTaskOfWay($oSeriesWay, $oIssue) {
        $aProjectIds = ManProject::getValidProjectIds($oIssue->lottery_id, $oIssue->issue, $oSeriesWay->id);
        return static::setSendCommissionTaskOfProjectIds($aProjectIds);
    }
     ...
     * @return bool
     */
    public static function setSendCommissionTaskOfProjectIds($aProjectIds) {
//        pr('add commission task: ' . var_export($aProjectIds,true));
        return static::addTask('SendCommission', ['projects' => $aProjectIds], 'commission');
//        return $this->pushJob('SendCommission',['projects' => $aProjectIds],Config::get('schedule.commission'));
    }
...
//            }
    ...
    protected function setStartSendMoneyTask() {
        return $this->addTask('StartSendMoney', $this->data, 'prize');
//        return $this->pushJob('StartSendMoney',$this->data,Config::get('schedule.prize'));
    }

    protected function setFinishedSendMoneyTask() {
//        pr('money finished');
        return $this->addTask('FinishSendMoney', $this->data, 'prize');
//        return $this->pushJob('FinishSendMoney',$this->data,Config::get('schedule.prize'));
    }
...
//        return $this->pushJob('FinishTracePrj',$this->data,Config::get('schedule.trace'));
    ...
    protected function compileOtherTasks() {
        ...
        //执行用户投注成功记奖后反积分
        $this->otherTasks[] = [
            'command' => 'SetAfterCalculatePoint',
            'queue' => 'point',
            'data' => [
                'lottery_id' => $this->data['lottery_id'],
                'issue' => $this->data['issue']
    ...
    function getDelayedTraces($oIssue) {
//        $this->hasWatingTraceDetail = TraceDetail::getUnGenerateDetailCount($oIssue->lottery_id,$oIssue->issue);
        $aTraces = TraceDetail::getUnGenerateTraces($oIssue->lottery_id, $oIssue->issue);
        $this->hasWatingTraceDetail = count($aTraces);
        foreach ($aTraces as $iTraceId) {
            $oTrace = ManTrace::find($iTraceId);
            if ($oTrace->status != Trace::STATUS_RUNNING) {
                continue;
            }
            if ($oTrace->prize > 0 && $oTrace->stop_on_won) {
                $this->needStopTraces[] = $iTraceId;
            } else {
                $this->needGenerateDetailTraces[] = $iTraceId;
            }
        }
    }
