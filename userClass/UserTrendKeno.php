...
//        return $result;
    ...
    public function generateTrendData(& $aWnNumbers, & $statistics, & $hotAndCold, & $aOmissionBarStatus, $iNumType)
    {
        ...
        $iSumStartCol = 11;
        $aData = [];
        $aTotalYiLou = array_fill(0,$iDataLength,0);
//        pr($aTotalYiLou);
//        exit;
//        $aTotalSumYiLou = array_fill($iMinSum,$iSumLength,0);
        foreach ($aWnNumbers as $aIssue) {
//            $aIssue['wn_number'] = K3Number::compileNumber();
//            $data = array_fill(0,$iDataLength,'');
            $aYiLou = array_fill(0,$iDataLength,1);
//            pr($aYiLou);
//            exit;
//            $aSumYiLou = array_fill($iMinSum,$iSumLength,0);
//            $data[0] = $sIssue = $aIssue['issue'];
//            $data[1] = $sNumber = $aIssue['wn_number'];
//            unset($data['issue']);
//            unset($data['wn_number']);

            $sNumber = $aIssue['wn_number'];
            $oNumber = new KenoNumber($sNumber);
            $aBalls = array_unique($oNumber->getDigitals());
//            pr($aBalls);
//            $aAttributes = $oNumber->getAttributes();
//            $iBallsLen = count($aBalls);
//            pr($data);
//             pr($aBalls);
//             exit;
...
//            pr($aYiLou);
//            exit;
            $aAllBalls = KenoNumber::getAllDigitals();
//            pr($aAllBalls);
//            pr($aBalls);
            $aDiff = array_diff($aAllBalls, $aBalls);
////
//            pr($aDiff);
////            exit;
//            pr($aBalls);
...
//                $value = intval($value);
                $iIndexOfYilou = intval($iNumber);
                $data[$iIndexOfYilou] = intval($iNumber);
                $aYiLou[$iIndexOfYilou] = 0;
...
//            pr($iIndexOfYilou);
            // 填充和值数据，含遗漏
            $iBaseIndexYilou = $iSumStartCol;
//            exit;
//            $iSum = $aAttributes['sum'];
//            $aTotalSumYiLou[$iSum] = 0;
//            for($i = $iMinSum; $i <= $iMaxSum;$i++){
//                $iIndexOfYilou = $iBaseIndexYilou + $i - $iMinSum;
...
//            pr($iIndexOfYilou);
//            exit;
//            $iIndexOfYilouOfSum = $iBaseIndexYilou + $iSum - $iMinSum;
//            $aTotalYiLou[$iIndexOfYilouOfSum] = 0;
////            $data[$iBaseIndexYilou + $iSum - 3] = $iSum;
//            $aYiLou[$iIndexOfYilouOfSum] = 0;
...
//            pr($aAttributes);
//            pr($oNumber->sumIsOdd());
//            pr($oNumber->sumIsSmall());
//            $aColumns = ['sumSmallOdd','sumSmallEven','sumBigOdd','sumBigEven','dragon0','dragon1','dragon2','dragon3','dragon4'];
